...
<body class="font-sans text-gray-900 antialiased">
    <div class="flex items-center justify-center h-screen bg-gray-100">
        <div class="w-full max-w-4xl bg-white rounded-lg shadow-lg overflow-hidden flex">
            ...
            <div class="w-1/2 p-10">
                ...
                <!-- Login Form -->
                <form method="POST" action="{{ route('pengelola.login') }}" id="loginForm">
                    @csrf
    ...
    <script>
        document.getElementById('user_type').addEventListener('change', function() {
            if (this.value === 'admin') {
                window.location.href = "{{ route('admin.login') }}"; // Ganti dengan route login untuk Admin Pengelola
            } else if (this.value === 'pengelola') {
                window.location.href = "{{ route('pengelola.login') }}"; // Ganti dengan route login untuk Pengelola
            }
             else if (this.value === 'user') {
                window.location.href = "{{ route('login') }}"; // Ganti dengan route login untuk Pengelola
            }
        });
    </script>
</body>
